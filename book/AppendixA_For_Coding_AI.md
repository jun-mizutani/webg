...
The first thing an AI should understand is that `webg` is not a thin wrapper around external libraries. It has its own design across rendering, scenes, models, animation, UI, input, diagnostics, and post-processing.
...
- Access after initialization: Access `app.space`, `app.eye`, `app.getGPU()`, and similar properties only after `await app.init()` has completed.
...
| Implement rigid-body style physics, bounce, or collider queries | Chapters 17 and 26 | `samples/physics_bounce`, `physics_space_contracts` |
| Build meshes or shaders at a low level | Chapter 22 and later | `low_level`, unittest files |
...
| Shape generation, vertices, material, wireframe, collision shape | `Primitive`, `Shape`, Chapters 22-24 |
| Texture, image loading, normal maps, procedural textures | `Texture`, Chapter 7, Chapter 24, `samples/proctex` |
...
| DOM overlays, help, long text, debug dock | `OverlayPanel`, `OverlayPanelPresets`, `DebugDock`, Chapters 14-15 |
| Raycast, picking, collision | `Space`, Chapter 17, `unittest/raycast` |

Most class names map to `webg/ClassName.js`, but there are exceptions. `formatJSON()` is in `webg/JsonFormat.js`, UI themes are in `webg/WebgUiTheme.js`, help / error option builders are in `webg/OverlayPanelPresets.js`, and `SkinningConfig` is a constants-and-functions module. If unsure, verify with:
...
- Controls and help: `app.showOverlayPanel(buildHelpPanelOptions(...))` or `app.showOverlayPanel({ collapsible: true, ... })`.
- Dynamic numbers and state: `app.message.setLines("status", [...], options)` or HUD rows through `setHudRows()`.
- Dialogue, tutorial text, UTF-8 text: `OverlayPanel` with `buttons` / `choices`, plus an application-side controller.
- Detailed information or error reasons: `app.showOverlayPanel({ format: "pre", scrollY: true, ... })` or `buildErrorPanelOptions()`.
...
- `SceneAsset`: Initial state for a whole scene, including camera, HUD, placed primitives, and placed models.
