...
本章では、`webg` のシェーダーを単に「機能として利用する」段階から一歩進め、`Shader.js` およびその派生クラスの実装をどのように読み解くべきかを解説します。対象となるファイルは、`webg/Shader.js`、`book/examples/Phong.js`、`book/examples/NormPhong.js`、`book/examples/BonePhong.js`、`book/examples/BoneNormPhong.js`、`webg/Background.js`、`webg/Font.js`、`webg/Wireframe.js`、および `webg/FullscreenPass.js` です。

第8章では WGSL の読解方法を扱いましたが、本章では CPU 側のクラス設計に重点を置きます。具体的には、「JavaScript 側で何を準備し、どのように GPU へ転送し、どのクラスがどのような役割を担っているか」という構造を明らかにします。なお、`Phong` 系 4 種類のファイルは、実装の差分を段階的に学習するためのサンプルとして提供されています。
...
また、実装の比較には `Phong` 系 4 種類が最適です。`Phong`、`NormPhong`、`BonePhong`、`BoneNormPhong` は、`webg` の 3D マテリアルを理解する上での核心部分です。「静的メッシュ」「ノーマルマップ」「スキニング」、そして「その両方の組み合わせ」という段階的な差分として捉えることで、実装の変更点を追いやすくなります。特に `Phong` は最小限の構成を持つ基本的なシェーダーであるため、分析の出発点として最適です。第7章では標準的なインターフェースとして `SmoothShader` を紹介しましたが、本章では実装の深い理解を得るため、`Phong` 系をベースにした差分分析を中心に構成します。

## `Shader.js` の共通構造

`webg` の多くのシェーダーは共通の処理フローを共有しています。具体的には、プロジェクション（projection）行列やモデルビュー（model-view）行列を受け取り、ユニフォームバッファを更新し、必要なテクスチャとサンプラーをバインドグループにまとめ、描画直前に GPU へ反映させるという流れです。これらの処理を個別のシェーダーごとに実装すると、機能追加やデバッグオプションの導入のたびにコードの重複と不整合が発生します。そこで、`Shader.js` はこれらの共通処理を引き受ける基底クラスとして機能しています。派生クラスは、この基底クラスを継承し、固有の WGSL コード、ユニフォームレイアウト、初期パラメータ、バインドグループレイアウト、およびパラメータを反映させるためのセッターメソッドを定義します。
...
特に注目すべきは、`default` と `change` という 2 つのパラメータ管理テーブルです。`default` はシェーダーの初期値を保持し、`change` は `Shape.shaderParameter()` によって現在上書きされている値を保持します。`doParameter(param)` メソッドは、指定されたキーをセッターへ流し、前回の呼び出しで上書きされていたが今回は指定されなかったキーを初期値へと戻す処理を行います。この設計により、ユーザーは毎回すべてのパラメータを再設定する必要がなく、必要な項目だけを更新することが可能です。この仕組みを理解していないと、「なぜ設定値が保持されないのか」あるいは「なぜ指定していない値が初期値に戻るのか」という挙動の原因が見えにくくなります。

シェーダーを拡張して新しいパラメータを追加する場合は、単に WGSL を書き換えるだけでは不十分です。初期値の設定、セッターメソッドの実装、`doParameter()` 内での反映処理、そして必要に応じて `setDefaultParam()` 側のインターフェース整備を行い、文書とサンプルコードに反映させる必要があります。これらの工程が漏れると、内部的にコードが存在していても、ユーザーが利用できない機能となってしまいます。実装を追加する際は、WGSL だけでなく、CPU 側のインターフェースと検証手段までを一貫して整備することが、`webg` の開発思想に沿った進め方です。
...
`SmoothShader` は、単に `Phong` 系の機能をすべて 1 ファイルへ足し合わせたものではありません。`Phong`、`NormPhong`、`BonePhong`、`BoneNormPhong` は学習用に差分を追いやすくするため個別のファイルに分けられていますが、`SmoothShader` はアプリ側で `shaderClass: SmoothShader` を指定するだけで、Shape ごとの material parameter によって描画経路を切り替えることを目的としています。たとえば、通常の形状では `has_bone: 0` のまま描画し、スキンドメッシュでは `has_bone: 1` を設定し、ノーマルマップを持つ形状だけ `use_normal_map: 1` と `normal_texture` を指定します。つまり、クラスを切り替えるのではなく、同じ `SmoothShader` の中で Shape ごとに必要な機能を選択する設計となっています。

`SmoothShader` の主な material parameter とその役割は以下の通りです。

### 色と質感の制御
`color` は material の基本色です。glTF の base color や、手動で設定したベース色として扱います。モデルアセット由来の色を保持したい場合、この値は元データの material 色として定義されます。
...
`use_texture` と `texture` は、ベーステクスチャを使用するか、およびどの texture を参照するかを指定します。glTF / GLB 由来の texture がある場合は loader 側で material parameter に反映されますが、手動で Shape を構築する場合はユーザー側で指定します。
...
### 機能的な切り替えとデバッグ
`has_bone` は、スキニング用の bone palette を使うかを指定します。スキンドメッシュでは `1` に設定し、通常の静的メッシュでは `0` のままにします。これにより、同一の `SmoothShader` 内でスキニング経路と非スキニング経路を切り替えます。
...
## `Phong` 系シェーダーの理論と差分解析

`Phong` 系 4 種類のシェーダーを段階的に比較することで、その構造がより鮮明になります。基本形である `Phong` に、ノーマルマップを加えたのが `NormPhong`、スキニングを加えたのが `BonePhong`、そしてその両方を統合したのが `BoneNormPhong` です。いきなり複雑な `BoneNormPhong` から読むのではなく、`Phong` $\rightarrow$ `NormPhong` $\rightarrow$ `BonePhong` $\rightarrow$ `BoneNormPhong` の順に差分を分析することを推奨します。

コンピュータグラフィックスにおいて照明を表現するには、物理現象を適切にモデル化（簡略化）する必要があります。`webg` では、物体を照らす光の計算を次の 3 つに分解して計算し、最後にそれらを合計する「Phong shading」を採用することで、効率的に視覚的な再現を行っています。
...
`Phong` は実用的な基本シェーダーであり、立方体や球、床などの基本オブジェクトに広く適用されます。ライティング、テクスチャ、フォグ、エミッシブ、スペキュラといった基本機能を網羅しており、スキニングやノーマルマップを必要としない場面に最適です。その役割は、モデルビュー変換された `position` と `normal` を用いて拡散反射と鏡面反射を計算し、ベーステクスチャを合成し、最終的にフォグを適用して色を決定することです。また、`backface_debug` を有効にすることで、裏面をマゼンタで描画するデバッグ機能も備えています。
...
この実装におけるポイントは、シーン全体で共有する光源情報はシェーダー側に保持し、オブジェクトごとの質感（マテリアル）は `shape.setMaterial()` を通じて Shape 側に渡している点です。開発者の視点では、`setProjectionMatrix()` や `setModelViewMatrix()` などの共通セッターと、Shape ごとの個別パラメータを区別して整理することが重要です。
...
`NormPhong` は `Phong` を理解した後に差分として分析するのが最も効率的です。注目すべき点は以下の 3 点です。

1. ノーマルマップ用テクスチャのバインディングが追加されているか。
2. `use_normal_map`、`normal_texture`、`normal_strength` を処理するセッターが実装されているか。
...
`NormPhong` は、表面の微細な凹凸感を表現するためのマテリアルです。実際のポリゴン数を増やさずに詳細な陰影を表現することを目的としています。ユーザー側では `use_normal_map` などのパラメータが追加されますが、`normal_texture` を指定しなかった場合はデフォルトの平坦なテクスチャが使用されるフォールバック機構が備わっています。そのため、「設定を ON にしたが外見が変わらない」場合は、テクスチャが正しく渡されているかを確認してください。
...
ユーザー側では、`has_bone: 1` を設定し、Shape 側にボーンインデックスとウェイトのデータが含まれていることが前提となります。`Shape.endShape()` は、スキニングメッシュの場合に専用のバッファを生成し、`BonePhong` のパイプラインはこれを前提とした構成になっています。スキニングが正しく動作しない場合は、まず `weight_debug` で頂点属性の正しさを切り分けることが推奨されます。
...
本シェーダーの役割は、「スキニングによる頂点変形」と「ノーマルマップによる法線制御」を同時に実現することです。また、`weight_debug` と `backface_debug` の両方を備えており、変形と陰影の両面から検証が可能です。ただし、`weight_debug` を有効にすると重みの色が優先して表示されるため、ライティングやノーマルマップの効果は確認できなくなります。目的に応じてこれらのデバッグモードを切り替えて分析してください。
...
`Wireframe` は、メッシュの面を塗るのではなく、`line-list` topology で edge を描くためのシェーダーです。描画入口は `shape.setWireframe(true)` であり、`Shape.draw()` は通常のシェーダーを使う代わりに、GPU ごとにキャッシュされた `Wireframe` インスタンスへ切り替えます。ここで重要なのは、`Wireframe` が「position だけを読む最小シェーダー」ではなく、`SmoothShader` と差し替えやすい vertex buffer layout と bind group layout を持つように作られている点です。

現在の `Wireframe` は、slot0 に position / normal / UV、slot1 に bone index / weight を受け取ります。線描画そのものには normal や UV は使いませんが、通常描画と同じ `Shape.draw()` の処理フローで vertex buffer を設定できるように、あえて `SmoothShader` と同じ入力形を受け入れます。非スキニングメッシュでは slot1 に 0 埋めのダミーバッファを渡し、スキニングメッシュでは Shape が作成した bone index / weight バッファをそのまま渡します。

bind group も同じ考え方です。`group(0)` は projection、modelView、線色、`has_bone` flag を含む draw ごとの uniform です。`group(1)` は `Wireframe` 自身では使用しませんが、`SmoothShader` の texture group と番号をそろえるため、空の bind group layout として pipeline layout に含めます。`group(2)` は bone palette で、`SmoothShader` と同様にスケルトンごとの uniform buffer / bind group を `WeakMap` でキャッシュします。`has_bone` が 0 の場合は共有の空 bone bind group を返すため、静的メッシュでは大きな palette の更新は発生しません。

WGSL 側の頂点シェーダーでは、`has_bone` が 0 の場合に identity 行列を使い、`has_bone` が 1 の場合に `input.index` と `input.weight` から最大 4 本の bone を合成します。bone palette は `SmoothShader` と同じく 1 bone を `vec4 x 3` として並べる形式なので、`i32(input.index.x) * 3` のようなオフセット計算で 3 行ぶんを取り出します。合成した `skinMat` を使い、最終的に次の順序で位置を変換します。
...
この構成により、静的メッシュとスキニングメッシュのどちらでも `Shape.setWireframe(true)` を同じ API で使えます。特に skinned mesh では、通常描画で見えている変形後の姿に wireframe が追従するため、bind pose の位置へ戻った線が重なるような不整合を避けられます。

wireframe の edge list は `Shape._buildWireIndexBuffer()` が作ります。`polygonLoops` がある場合は、三角形化された描画用 index ではなく、元の face loop の外周から線を作ります。そのため `Shape.addPolygon([a, b, c, d])` や `addPlane()` で作った四角面は、内部の対角線を表示せず、四角形の外周だけが表示されます。`polygonLoops` がない場合は、入力が三角形メッシュであると見なし、triangle edge をそのまま wireframe として扱います。三角形済みデータから四角面を推定して共有辺を消す処理は行いません。これは、入力 geometry の意味をシェーダー側で勝手に変えないためです。
...
特に `doParameter()` への反映を忘れると、`Shape.setMaterial()` でパラメータを指定しても反映されないという不具合が発生します。これは視覚的に気づきにくく、原因の切り分けに時間を要するため注意が必要です。`webg` のシェーダーは、「CPU 側のクラス設計 $\rightarrow$ ユニフォーム更新 $\rightarrow$ バインドグループ $\rightarrow$ GPU 描画」までが一連のパイプラインとして機能しています。変更を加える際は、この一連の流れに断絶がないかを確認してください。

## コーディング AI によるシェーダー修正時の注意点
...
この順序で学習することで、サンプルコードを分析する際にも迷いなく実装を追跡できます。
