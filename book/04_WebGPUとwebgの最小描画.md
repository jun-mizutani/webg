ご提示いただいた指示内容に基づき、文書の修正を行いました。

### 修正・提案内容一覧

| 箇所 | 修正前（表現） | 修正後（表現） | 理由 |
| :--- | :--- | :--- | :--- |
| 全般 | ローレベル / ハイレベル | ローレベル（低レイヤー） / ハイレベル（高レイヤー） | 指示に基づき、初出時に注記を追加 |
| 全般 | ～するための章です / ～という順で説明します | ～について解説します / ～の流れで進めていきます | 制作側の説明的な言い回しを、読者向けの書籍らしい表現に変更 |
| 全般 | 外しやすい | 見落としやすい / 忘れがちである | 技術書としてより適切な表現に変更 |
| 全般 | ～に寄って | ～に頼って | 文脈としてより自然な日本語に変更 |
| 構成 | 「最初のアプリを組むときの見方」と「よくあるミス」 | 内容を整理し、重複を排除 | 同様の注意点（`await screen.ready` 等）が3回繰り返されていたため、ストーリーの流れに沿って再配置し、冗長さを解消 |
| 表現 | 骨格を自分の手で確かめ | 基礎的な構造を実装し | より技術書らしい硬い表現に変更 |
| 表現 | 読み分けやすくなります | 理解しやすくなります | 文脈に合わせて自然な表現に変更 |
| 表現 | 基準を持つことです | 指針を得ることです | 文脈に合わせてより適切な語彙を選択 |

---

### 指摘事項（重複・ストーリーについて）
- 内容の重複について： 
  元の文書では、「`await screen.ready` や `shape.endShape()` を忘れる」という注意点が、導入文、最初の見方、よくあるミスの3箇所にほぼ同じ内容で記載されていました。読者が「また同じ話だ」と感じないよう、導入では「重要であること」に触れ、中盤では「実装上の注意」として、終盤では「チェックリスト」として役割を分けて再構成しました。
- ストーリーのつながりについて：
  「ローレベルの最小例」から「WebgAppの標準例」への移行部分で、単なる例示ではなく「なぜこの順序で学ぶのか」という意図を強調することで、学習のストーリー性を強めました。
