...
- `setTint(r, g, b, a = 1.0)`: 周辺へ寄せる色を更新する
...
`Touch` と `InputController` は、キーボードとタッチを同じ key state に寄せるための層です。
...
### `InputController`
`InputController` は、keyboard、pointer、touch button を同じ key state へ寄せる class です。
