...
つまり core の API 一覧としては、会話専用 class や `WebgApp.startDialogue()` のような facade は持たず、文字表示系は `Text` / `Message` / `OverlayPanel` / `DebugDock` の組み合わせで説明する
...
- `validateTileMap(tileMap, path)`: TileMap 定義を検証する
...
確認対象は `camera`、`hud`、`input`、`tileMap`、`primitives`、`models` です。
...
- `buildTileMap(tileMapDef)`: tileMap 定義を `TileMap` runtime へ変換する
...
`build(scene)` の戻り値は `entries`、`tileMap`、`inputMap`、`scene`、`update()`、`createInputHandler(actionHandlers)`、`getEntry(id)` を持ちます。
...
- `clearGameHud()`: score / combo / timer / toast などの game HUD 状態を消す
- `setScore(value, options = {})`: score 表示を設定する
- `addScore(delta = 0, options = {})`: 現在の score に加算する
- `setCombo(value, options = {})`: combo 表示を設定する
- `setTimer(value, options = {})`: timer 表示を設定する
...
- `createHelpPanel(options = {})`: help panel を作成する
- `setHelpPanelLines(helpPanel, lines = [])`: help panel の本文行を差し替える
- `setHelpPanelVisible(helpPanel, visible = true)`: help panel の表示 / 非表示を切り替える
- `clearHelpPanel(helpPanel)`: help panel を 1 枚消す
- `clearAllHelpPanels()`: help panel を全消去する
...
- `triggerAction(name)`: `pulseAction()` と同じ意味の旧名
...
- `constructor(space, gpu, definition = {})`: `Space` / GPU と盤面定義を受けて初期化する
- `static fromScene(scene = {}, space, gpu)`: Scene JSON の `tileMap` 定義から `TileMap` を作る
- `clamp(value, min, max)`: 値を範囲内へ収める
- `terrainFromHeight(height)`: 高さから terrain id を決める
- `colorFromTerrain(terrain)`: terrain id から既定色を返す
- `resolveTerrainMaterial(terrain, cell = null)`: terrain ごとの材質定義を返す
- `tintColor(rgba, factor)`: 基本色へ明暗補正をかける
- `normalizeDimensions(definition)`: 行数、列数、cell 寸法を正規化する
- `normalizeDisplayArea(displayArea)`: `displayArea` を現在の盤面寸法へ合わせて整える
- `generateTileDefinitions(definition)`: `tiles` と `generator` から cell 定義群を作る
- `createCellSpec(col, row, height, terrain = null)`: 1 cell 分の論理定義を作る
- `createCellMaterial(cell)`: cap 用の材質情報を作る
- `createCellFoundationMaterial(cell)`: foundation 用の材質情報を作る
- `applyCellMaterial(shape, cellMaterial)`: 1 cell 用材質を shape に反映する
- `assignEdgeCutMasks()`: 近傍との高さ差から全 cell の edge cut mask を計算する
- `applySurfaceShapes()`: 盤面形状を scene へ配置する
- `build()`: TileMap 全体の shape / node / metadata をまとめて構築する
- `getCell(col, row)`: 指定座標の cell を返す
- `getTile(cellOrX, rowMaybe = undefined)`: cell 参照または座標から tile 定義を返す
- `getTopY(col, row)`: 指定 cell 上面の Y 座標を返す
- `getWorldPosition(cell)`: 指定 cell の world 座標を返す
- `getHeight(cell)`: 指定 cell の高さ段数を返す
- `toWorld(cell)`: cell 座標を world 座標へ変換する
- `getEdgeCutMask(cell)`: 指定 cell の edge cut mask を返す
- `getShapeKind(cell)`: 指定 cell の形状種別を返す
- `isBlocked(cell)`: その cell が通行不可かを返す
- `isInDisplayArea(col, row, displayArea = this.displayArea)`: 表示窓に含まれるかを返す
- `setDisplayArea(displayArea)`: 表示窓を更新する
- `getDisplayAreaCells(displayArea = this.displayArea)`: 指定表示窓の cell 一覧を返す
- `followCell(cell, options = {})`: 指定 cell が見えるよう表示窓を追従させる
- `getNodePositionOnCell(cell, lift = 0.04, radius = 0.82)`: node を置くべき座標を計算する
- `placeNodeOnCell(node, cell, lift = 0.04, radius = 0.82)`: node を指定 cell 上へ配置する
- `refreshTileColors(selected = null)`: terrain / material 変更後に cell 色を更新する
- `intersectRayAabb(origin, dir, box)`: ray と AABB の交差距離を返す
- `pickCell(origin, dir)`: ray と盤面の交差から cell を選ぶ
- `canMove(fromCell, toCell)`: 2 cell 間を移動可能か判定する
- `findPath(start, goal)`: 盤面上の経路探索を行う
