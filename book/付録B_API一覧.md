...
### `FrostedGlassPass`
`FrostedGlassPass` は、scene color、blur color、glass mask を合成し、曇りガラスのように背後がぼけて見える領域を作るポストプロセスです。`unittest/translucent` の中心です。

- `constructor(gpu, options = {})`: 曇りガラス用の scene / mask / blur / composite helper を初期化する
- `resize(width, height)`: scene / mask / blur targets を指定サイズへ更新する
- `resizeToScreen(screen)`: `Screen` の現在サイズへ追従する
- `beginScene(screen, clearColor = screen.clearColor)`: 不透明 scene を描く offscreen pass を始める
- `beginMask(screen, clearColor = [0, 0, 0, 0])`: scene depth を使いながら glass mask pass を始める
- `render(screen, options = {})`: scene を blur し、scene / blur / mask を合成して canvas または destination に出力する
- `setEnabled(flag)`: 曇りガラス効果の ON / OFF を切り替える
...
- `setBlurStrength(value)`: mask alpha による blur 混合量を更新する
- `setTintStrength(value)`: mask RGB の tint を最終色へ混ぜる強さを更新する
- `setMaskPower(value)`: mask alpha の効き方を指数的に調整する
- `getSceneTarget()`: 不透明 scene の offscreen target を返す
- `getMaskTarget()`: glass mask の target を返す
- `getBlurTarget()`: 直近 render の blur 出力 target を返す
- `getBlurTargetA()`: blur helper の一時 target A を返す
- `getBlurTargetB()`: blur helper の一時 target B を返す

### `SeparableBlurPass`
`SeparableBlurPass` は、横 blur と縦 blur を ping-pong するヘルパーです。bloom、被写界深度（DOF）、曇りガラスの blur で使います。
...
### `GlassMaskShader`
`GlassMaskShader` は、`FrostedGlassPass` の mask target へガラス領域を書き込むための専用 shader です。通常の質感描画ではなく、mask の RGB と alpha を composite pass へ渡す役割を持ちます。

- `constructor(gpu, options = {})`: mask shader の target format、depth compare、cull mode などを初期化する
- `setProjectionMatrix(m)`: projection matrix を uniform へ設定する
- `setModelViewMatrix(m)`: model-view matrix を uniform へ設定する
- `setNormalMatrix(m)`: `Shape.draw()` から法線行列を受け取る入口
- `setColor(color)`: mask target へ書く RGB tint と alpha 強度を設定する
- `doParameter(param = {})`: `color` または `mask_color` を shader uniform へ反映する
...
- `releaseObjects()`: CPU 側の頂点配列や補助配列を空にする
- `destroy(options = {})`: shape instance を終了し、必要に応じて shared resource の破棄へつなげる
...
- `addPolygon(indices)`: 多角形を追加し、wireframe 用には元の辺ループも保持する
...
- `setPositionX(x)`: X 位置だけを設定する
- `setPositionY(y)`: Y 位置だけを設定する
- `setPositionZ(z)`: Z 位置だけを設定する
- `getScale()`: local uniform scale を返す
- `setScale(scale)`: local uniform scale を設定する
...
- `composeMatrixFromState(matrix, quat, position, scale = 1.0)`: TRS から local matrix を合成する
- `getRigidMatrix(matrix)`: uniform scale を除いた剛体行列を返す
- `decomposeMatrixTransform(matrix)`: matrix を position / quat / uniform scale に分解する
...
- `getQuatFromMatrix()`: 現在 matrix から quaternion を算出する
- `getPositionFromMatrix()`: 現在 matrix から position を抽出する
- `detach()`: 親から切り離す
...
- `putRotationByQuat(quat)`: 補間用の相対 quaternion 回転を設定する
- `putAttitudeByQuat(quat)`: 補間用の絶対 quaternion 姿勢を設定する
- `putAttitude(yaw, pitch, roll)`: 補間用の絶対 Euler 姿勢を設定する
- `putDistance(x, y, z)`: 補間用の移動差分を設定する
- `putRotTrans(quat, pos)`: 補間用の回転と移動を同時設定する
- `putRotTransByMatrix(matrix)`: matrix から補間用の回転と移動を設定する
- `putMatrixByMatrix(matrix)`: matrix 補間用の開始 / 終了行列を設定する
- `execRotation(t)`: 補間率で回転を適用する
- `execTranslation(t)`: 補間率で移動を適用する
- `doRotation(t)`: `execRotation()` 後に matrix を更新する
- `doTranslation(t)`: `execTranslation()` 後に matrix を更新する
- `doRotTrans(t)`: 回転と移動を同時に補間適用する
- `doMatrix(t)`: matrix 補間を適用する
...
- `setParent(parent)`: 親 node を設定する
- `hide(true_or_false)`: node 配下 shape の表示を切り替える
- `setAttachable(true_or_false)`: attach 可否を設定する
- `setWeights()`: この node が weight を持つことを記録する
- `detach()`: 親から切り離す
...
- `animatePosition(to, options = {})`: local position を tween で補間する
- `updateAnimatedPosition(deltaMs = 0)`: position tween を進める
- `clearAnimatedPosition()`: position tween を消す
- `animateRotation(to, options = {})`: local rotation を tween で補間する
- `updateAnimatedRotation(deltaMs = 0)`: rotation tween を進める
- `clearAnimatedRotation()`: rotation tween を消す
...
- `removeNode(node, options = {})`: node 1 個を scene graph から外す
- `removeNodeTree(node, options = {})`: node subtree 全体を scene graph から外し、必要なら配下 shape も終了する
...
- `draw(eye_node, options = {})`: node 木を描画する。`options.filter({ node, shape, index })` を渡すと pass ごとに描画対象を選別できる
...
### `PhysicsNode`
`PhysicsNode` は、`Node` を継承した物理ノードです。scene graph と描画ノードの構造を保ったまま、`bodyType`、質量、速度、sleep、collider 参照をまとめて持たせるための入口として使います。

現行の設計では、`PhysicsNode` 自身は「1 個の物体の状態」を保持し、複数物体の接触解決や跳ね返りは `PhysicsSpace` 側で扱います。`dynamic` 中の transform 直接変更は許可せず、位置を変えたいときは `teleport()` を使うか、いったん `kinematic` へ切り替えてから操作する前提です。
...
- `resetInertia()`: collider と質量から自動計算する inertia へ戻す
...
- `setTrigger(enabled)`: trigger 扱いにするか設定する
- `getTrigger()`: trigger 状態を返す
- `setFixedRotation(enabled)`: 回転固定を設定する
- `getFixedRotation()`: 回転固定状態を返す
- `setCollisionLayer(layer)`: collision layer bitmask を設定する
- `getCollisionLayer()`: collision layer bitmask を返す
- `setCollisionMask(mask)`: 接触対象にする layer bitmask を設定する
- `getCollisionMask()`: collision mask bitmask を返す
- `canCollideWith(otherBody)`: 双方の layer / mask が一致し、接触候補にできるかを返す
- `setCollider(collider)`: collider 参照を保持する
- `getCollider()`: collider 参照を返す
...
- `setLinearVelocityVec(velocity)`: vec3 で線形速度を設定する
- `getLinearVelocity()`: 線形速度を返す
- `setAngularVelocity(x, y, z)`: 角速度を設定する
- `setAngularVelocityVec(angularVelocity)`: vec3 で角速度を設定する
...
- `stopMotion()`: 線形速度、角速度、accumulator を 0 に戻す
...
### `PhysicsSpace`
`PhysicsSpace` は、複数の `PhysicsNode` をまとめて進める物理空間です。固定タイムステップ、重力、plane collider と box collider の接触解決、box-box の OBB 接触解決、反発、摩擦、sleep 判定、contact begin / stay / end の記録、および各段階の listener 通知までを扱います。`PhysicsNode` が trigger に設定されている場合は、contact 自体は検出して event / listener に流しますが、押し戻しと速度反発は行いません。問い合わせ系としては、同じ collider 情報を使う `raycast()`、`raycastAll()`、`queryAabb()`、`overlapSphere()` も備えています。

立方体同士の衝突や跳ね返りは `PhysicsNode` ではなく `PhysicsSpace.step()` の内部で解決します。plane collider に対する着地や滑り、plane / box / sphere / capsule collider に対する物理 raycast、AABB 範囲問い合わせ、球範囲問い合わせもここで扱います。`raycast()` は最短 1 件、`raycastAll()` は距離順の全 hit を返します。box は body の quaternion 姿勢を反映した OBB として narrowphase / query / raycast に参加し、broadphase では外接 AABB を使います。将来的に joint を追加するとしても、その責務はここへ集約する前提です。
...
- `setMaxSubSteps(value)`: 1 frame あたりの最大 sub step 数を設定する
- `getMaxSubSteps()`: 1 frame あたりの最大 sub step 数を返す
- `setSolverIterations(value)`: contact solver の反復回数を設定する
- `getSolverIterations()`: contact solver の反復回数を返す
- `setBroadphaseMode(mode)`: broadphase mode を `"sweepAabb"` または `"bruteForce"` に設定する
- `getBroadphaseMode()`: broadphase mode を返す
...
- `setSleepStepsThreshold(value)`: sleep に入るまでに必要な連続低速 contact step 数を設定する
- `getSleepStepsThreshold()`: sleep に入るまでに必要な連続低速 contact step 数を返す
- `resetAccumulator()`: accumulator を 0 に戻す
...
- `getBodies()`: 現在登録されている body 一覧を返す
- `getLastContacts()`: 直近の fixed step 全体で解決した contact 一覧を返す
- `getLastContactEvents()`: 直近の fixed step における接触開始、継続、終了を `begin / stay / end` で返す
- `onBeginContact(listener)`: 接触開始時の listener を登録する
- `onStayContact(listener)`: 接触継続時の listener を登録する
- `onEndContact(listener)`: 接触終了時の listener を登録する
- `offBeginContact(listener)`: 接触開始 listener を解除する
- `offStayContact(listener)`: 接触継続 listener を解除する
- `offEndContact(listener)`: 接触終了 listener を解除する
- `raycast(origin, dir, options = {})`: plane / box / sphere / capsule collider に対する最短 hit を返す。`layerMask` で対象 layer を絞れ、`triggerOnly` で trigger body だけを対象にできる
- `raycastAll(origin, dir, options = {})`: plane / box / sphere / capsule collider に対する全 hit を距離順で返す。`layerMask` で対象 layer を絞れ、`triggerOnly` で trigger body だけを対象にできる
- `queryAabb(min, max, options = {})`: physics space AABB と重なる AABB 対応 collider 一覧を返す。`layerMask` で対象 layer を絞れ、`triggerOnly` で trigger body だけを対象にできる
- `overlapSphere(center, radius, options = {})`: sphere と重なる collider 一覧を返す。`layerMask` で対象 layer を絞れ、`triggerOnly` で trigger body だけを対象にできる
- `step(deltaMs)`: 可変 delta を内部 accumulator へ積み、必要回数だけ fixed step を進める
- `stepFixed(dtSec)`: 1 回分の固定 timestep で重力、接触解決、摩擦、sleep 判定まで進める

### `Collider` / `BoxCollider` / `PlaneCollider` / `SphereCollider` / `CapsuleCollider`
`Collider` は、`PhysicsNode` に設定する物理判定形状の基底クラスです。`PhysicsSpace` は collider の `getBroadphaseKind()`、`canBroadphasePairWith()`、`buildContactWith()`、`intersectRay()`、`overlapsAabb()`、`overlapSphere()` を通じて、形状ごとの broadphase / narrowphase / query / raycast を dispatch します。

- `Collider(type, options = {})`: collider 種別と `offset` を持つ基底 collider を作る
- `getWorldPosition(position, quat = null)`: body 位置と `offset` から collider の world 位置を返す。`quat` を渡した場合は offset も姿勢に合わせて回す
- `getBroadphaseKind()`: broadphase 用の種別名を返す
- `canBroadphasePairWith(otherCollider)`: broadphase 候補にする相手 collider かを返す
- `intersectRay(position, origin, dir, maxDistance = Infinity, quat = null)`: ray hit 情報、または `null` を返す
- `overlapsAabb(position, queryMin, queryMax, quat = null)`: query AABB と重なるかを返す
- `getAabb(position, quat = null)`: collider の world AABB、または `null` を返す
- `overlapSphere(position, center, radius, quat = null)`: sphere overlap 情報、または `null` を返す
- `buildContactWith(position, otherCollider, otherPosition, bodyA, bodyB, quat = null, otherQuat = null)`: 相手 collider 型に応じた contact 生成へ dispatch する

`BoxCollider(size, options = {})` は body の quaternion 姿勢を反映する OBB collider です。broadphase では外接 AABB を返し、box-box、plane-box、sphere-box、capsule-box の contact、ray-box、queryAabb、overlapSphere に対応します。

`PlaneCollider(normal, options = {})` は無限平面 collider です。plane-box、plane-sphere、plane-capsule の contact と ray-plane に対応します。AABB を持たないため、`queryAabb()` の結果には入りません。

`SphereCollider(radius, options = {})` は球 collider です。sphere-sphere、sphere-box、plane-sphere、capsule-sphere の contact、ray-sphere、queryAabb、overlapSphere に対応します。

`CapsuleCollider(radius, segmentLength, options = {})` は y 軸方向の capsule collider です。capsule-capsule、capsule-sphere、capsule-box、plane-capsule の contact、ray-capsule、queryAabb、overlapSphere に対応します。

### `Skeleton`
`Skeleton` は bone tree と matrix palette を管理します。現行の skinned mesh 描画では `SmoothShader` のスキニング経路と組み合わせて使います。
...
### `Primitive`
`Primitive` は、`cube`、`sphere`、`arrow` などの基本形状を `ModelAsset` として生成する static factory です。`samples/model_shape`、`samples/scene`、`unittest/primitive_modelasset` でよく使います。

生成結果は `ModelAsset` なので、`Shape.applyPrimitiveAsset(asset)` へ渡すか、`ModelAsset.build(gpu)` でランタイム形状へ変換します。

- `static getOptions(options = {})`: UV mapping 用 option を既定値込みで正規化する
- `static makeAsset(name, geometry)`: 単一 mesh / 単一 node の `ModelAsset` を作る
- `static makeRevolutionGeometry(latitude, longitude, verts, spherical, options = {})`: 回転体用 geometry object を作る
...
- `static mapCuboid(size_x, size_y, size_z)`: UV 展開済み直方体を作る
- `static cube(size, options = {})`: 立方体を作る
- `static mapCube(size)`: UV 展開済み立方体を作る
- `static debugBone(a, options = {})`: bone debug 表示向け形状を作る
...
- `matToEulerXYZ()`: XYZ 順 Euler 角へ変換する
...
- `getAxisScale()`: 各 local 軸の scale を返す
- `getUniformScale(epsilon = 1.0e-4)`: uniform scale なら値を返し、非一様なら null を返す
- `applyUniformScale(scale)`: 3x3 回転部分へ uniform scale を掛ける
- `removeUniformScale(scale)`: uniform scale を除いた行列を返す
- `add(mb)`: 行列加算を行う
- `mul(mb)`: 右側の行列を掛ける
- `lmul(mb)`: 左側の行列を掛ける
...
- `eulerToQuat(yaw, pitch, roll)`: Euler 角を quaternion に変換する
...
ここは、3D の上に texture、文字、操作ガイド、長文 panel を重ねるための層です。

`Texture` は画像や手続き生成の入口で、`Text` と `Message` は canvas 上の HUD を作ります。

`OverlayPanel` と `DebugDock` は canvas 外の DOM を使う UI 層です。help、error、briefing、log の違いは class 名ではなく `OverlayPanel` の option で表します。
...
- `readImageFromFile(textureFile)`: 画像ファイルを読み込んで texture 化する
- `readNormalMapFromFile(textureFile)`: 法線マップ画像を読み込んで texture 化する
- `makeNormalMapPixelsFromHeightMap(options = {})`: height map 画像から normal map の pixel 配列を作る
- `buildNormalMapFromHeightMap(options = {})`: 現在の height map texture から normal map texture を作る
- `readNormalMapFromHeightFile(heightMapFile, options = {})`: height map ファイルを読んで normal map texture を作る
...
- `makeProceduralHeightMapPixels(options = {})`: 手続き的な height map pixel 配列を作る
- `buildProceduralHeightMap(options = {})`: 高さマップを手続き的に作る
- `buildNormalMapFromProceduralHeight(options = {})`: 手続き的 height map から normal map texture を作る
- `makeProceduralBillboardTexturePixels(options = {})`: billboard 用の手続き的 pixel 配列を作る
...
一方で、日本語や一般的な UTF-8 文章を出す用途には向いていません。その場合は `OverlayPanel` を使うのが標準です。
...
### 会話 UI の扱い
会話、チュートリアル本文、選択肢のような UI 専用 helper は core から削除した

現在の方針では、UTF-8 の本文や button 付き panel は `OverlayPanel` を使って app / sample 側で組み立てる

つまり core の API 一覧としては、会話専用 class や用途別 facade は持たず、文字表示系は `Text` / `Message` / `OverlayPanel` / `DebugDock` の組み合わせで説明する
...
### HTML panel 系
`OverlayPanel`、`OverlayPanelPresets`、`DebugDock`、`WebgUiTheme` は、HTML ベースの補助 UI を作る層です。

`OverlayPanel` は scene の上に重ねる text / button / choice panel です。help、error、briefing、log は option で表します。`DebugDock` は PC 向けの開発補助表示です。

`unittest/overlay_panel`、`unittest/embedded`、`unittest/theme` が代表例です。
...
- `OverlayPanel`: scene の上に重ねる DOM panel です。`constructor()` で panel を作り、`update()` で option を差し替え、`show()` / `hide()` / `remove()` で表示を制御し、`setTheme()` で見た目を更新し、`setCollapsed()` で本文を畳み、`getState()` で状態を返します。`anchor`、`format`、`scrollY`、`buttons`、`choices`、`modal`、`pauseScene` などの option で用途を表します
- `OverlayPanelPresets`: `buildHelpPanelOptions()` と `buildErrorPanelOptions()` で代表的な option object を作ります。これは `WebgApp` の用途別 facade ではなく、必要な sample / book example が明示 import する helper です
...
- `async loadJSON(source)`: ModelAsset JSON を読む
- `async loadGltf(source, options = {}, onStage = null)`: glTF / GLB を読む
- `async loadCollada(source, options = {}, onStage = null)`: Collada を読む
- `async loadAsset(source, options = {})`: 形式ごとの差を吸収して `ModelAsset` を読む
- `async load(source, options = {})`: 形式を吸収して 1 本の loader として実行する

`load(source, options)` は `fetch -> validate -> build -> apply-runtime-materials -> instantiate -> runtime` の流れで進み、戻り値に `asset`, `runtime`, `instantiated`, `getClipNames()`, `getClipInfo(id)`, `instantiate()`, `downloadJSON()` を持ちます。`instantiated` は scene 上に配置された実体、`runtime` は共有 resource を保持する基盤と考えると、寿命管理を整理しやすくなります。
...
- `static fromData(data)`: data から asset を作る
- `static fromJSON(text)`: JSON 文字列から asset を作る
- `static isGzipSource(source)`: URL / file 名が `.json.gz` かを判定する
- `static assertCompressionStreamSupport(operation = "gzip ModelAsset JSON")`: gzip 入出力に必要な browser API が使えるか確認する
- `static async compressTextToGzipBlob(text)`: JSON 文字列を gzip Blob へ変換する
- `static async decompressGzipBlobToText(blob)`: gzip Blob を JSON 文字列へ復元する
- `static async fromGzipBlob(blob)`: gzip 圧縮された ModelAsset JSON Blob から asset を作る
- `static async load(url)`: URL から JSON または `.json.gz` の asset を読む
...
- `async toJSONGzBlob(indent = 2)`: 現在の JSON を gzip Blob として返す
- `static downloadBlob(blob, filename)`: Blob を指定ファイル名で保存する
- `downloadJSON(filename = "modelasset.json", indent = 2)`: JSON をファイル保存する
- `async downloadJSONGz(filename = "modelasset.json.gz", indent = 2)`: gzip 圧縮 JSON をファイル保存する
...
`build(asset)` の戻り値には、`materialDefs`、`meshDefs`、`skeletonDefs`、`animationMap`、`nodes`、`nodeMap`、`shapes`、`createNodeTree(space)`、`bindAnimationBindings()`、`instantiate(space, options)`、`destroy()`、および animation helper が入ります。
...
- `static fromData(data)`: data から asset を作る
- `static fromJSON(text)`: JSON 文字列から asset を作る
- `static async load(url)`: URL から scene を読む
- `setData(data)`: 内部 data を差し替える
- `getData()`: 内部 data を返す
- `toJSONText(indent = 2)`: JSON 文字列へ書き出す
...
- `async build(target)`: 実行環境へ scene をビルドする
...
- `ensureObject(value, path, label)`: object であることを確認する
...
- `validateTransform(transform, path)`: transform の translation / rotation / scale を検証する
- `validateMaterialOverride(material, path)`: material override を検証する
- `validateCamera(camera, path)`: camera の `target`, `distance`, `yaw`, `pitch`, `roll` などを検証する
- `validateHud(hud, path)`: HUD 定義を検証する
- `validateInput(input, path)`: input bindings を検証する
- `validatePrimitive(entry, path)`: primitive entry を検証する
- `validateModel(entry, path)`: model entry を検証する
- `validateScene(scene)`: scene 全体の内部検証を行う
...
- `normalizeHudLines(lines = [])`: HUD 行を整える
...
- `ensurePhysicsSpace(scene = {})`: scene 用 `PhysicsSpace` を用意する
- `buildPhysicsCollider(colliderDef = {})`: scene の collider 定義から collider instance を作る
- `createPhysicsPlacementNode(entry, defaultName, scene)`: `entry.physics` 付き entry を `PhysicsNode` として作る
- `applyMaterialOverride(shape, material = {})`: shape へ material 上書きを反映する
- `buildPrimitiveAsset(entry)`: primitive entry をアセット化する
- `async resolveModelAsset(entry)`: model entry からアセットを解決する
...
- `async buildEntryRuntime(entry, asset)`: 1 entry 分のランタイムを組み立てる
- `createInputMap(scene)`: scene の input map を作る
- `async build(scene)`: Scene JSON 全体をランタイムに変換する
...
Scene JSON で `physicsSpace` を top-level に持たせると `PhysicsSpace` を生成でき、各 primitive / model entry に `physics` を持たせると、その placement node を `PhysicsNode` として作成できます。`entry.physics.collider` を必須にし、`bodyType`、速度、減衰、collision layer / mask、`trigger`、`fixedRotation`、`material.restitution / friction`、`box / plane / sphere / capsule` collider を明示する構成です。`stepPhysics(deltaMs)` は build 後の runtime から明示的に呼び出し、Scene JSON で宣言した物理空間をアプリケーション側の更新処理から進めます。
...
- `setAngles(yaw, pitch, roll = 0.0)`: 向き角を設定する
- `setLookAngles(yaw, pitch, roll = 0.0)`: 視線角を設定する
...
- `requireReport(report, methodName)`: diagnostics report object であることを確認する
- `requirePlainObject(value, methodName, name)`: object 引数を検証する
- `requireArray(value, methodName, name)`: array 引数を検証する
- `requireInputFrame(frame, methodName, index = null)`: input replay frame を検証する
- `toSummaryText(report, options = {})`: 人と AI が最初に読む summary text へ整形する
- `getSummaryStatKeys(stats = {})`: summary で優先表示する stat key の順序を返す
...
- `summarizeInputFrame(frame, index = 0)`: 入力 frame を 1 行に要約する
...
- `copySummary(report, options = {})`: summary text を clipboard へ送る
...
- `async loadModel(source, options = {})`: Model loader の高レベル入口としてモデルを読む
- `async loadScene(scene)`: Scene loader の高レベル入口としてシーンを読む
...
- `updateTweens(deltaMs = 0)`: 登録済み tween を進め、終了済み tween を取り除く
- `clearTweens()`: 登録済み tween をすべて消す
- `async createParticleEmitter(options = {})`: particle emitter を作成し、必要なら renderer も初期化する
- `updateParticleEmitters(deltaMs = 0)`: 登録済み particle emitter を更新する
- `drawParticleEmitters()`: 登録済み particle emitter を描画する
- `clearParticleEmitters()`: 登録済み particle emitter を消す
- `shakeCamera(options = {})`: camera shake を開始する
- `updateCameraEffects(nowMs = Date.now())`: camera shake や follow target を反映する
- `updateCameraTarget(deltaMs = 0)`: follow / lock-on target から camera target を更新する
- `followNode(node, options = {})`: node を camera target として追従する
- `lockOn(target, options = {})`: node または座標を注視 target にする
- `clearCameraTarget()`: camera の follow / lock-on target を解除する
...
- `updateOverlayPanel(panelOrId, patch = {})`: `OverlayPanel` の option を差し替える
- `hideOverlayPanel(panelOrId)`: `OverlayPanel` を非表示にする
- `removeOverlayPanel(panelOrId)`: `OverlayPanel` を DOM と管理表から削除する
...
- `reportRuntimeWarning(line, options = {})`: runtime warning を記録して diagnostics へ反映する
...
- `copyDiagnosticsSummary(options = {})`: diagnostics summary を clipboard へコピーする
- `copyDiagnosticsReportJSON(options = {})`: diagnostics JSON を clipboard へコピーする
...
- `getCurrentDiagnosticsReport(options = {})`: 現在の scene / runtime 状態を反映した report を返す
- `getCurrentStateDockText(options = {})`: debug dock 用の現在状態 text を返す
- `invalidateCurrentDiagnosticsCache(options = {})`: diagnostics cache を無効化する
- `buildAutomaticDiagnosticsStats(report, options = {})`: camera / canvas / scene などの stat を自動収集する
- `mergeAutomaticDiagnosticsContext(report, options = {})`: diagnostics context を自動補完する
- `collectCurrentSceneStats(options = {})`: 現在 scene の node / shape / animation 統計を集める
- `setLatestRuntimeError(error, options = {})`: 直近 runtime error を記録する
- `setLatestRuntimeWarning(line, options = {})`: 直近 runtime warning を記録する
- `clearLatestRuntimeAlerts()`: runtime error / warning 表示を消す
- `syncLatestRuntimeAlertsFromReport(report)`: diagnostics report から runtime alert 表示を同期する
...
- `getProgressStorage()`: progress 保存に使う storage を返す
- `getProgressStorageKey(key)`: progress 保存用の実 key を返す
...
音周りは、`ToneSynth` が単音基盤、`AudioSynth` が SE/BGM 基盤、`GameAudioSynth` がゲーム向けの便利層です。
...
- `ensureContext()`: AudioContext を確保し、単音用 bus と FX chain を準備する
- `buildToneFxChain()`: tone bus 用の delay / reverb chain を作る
- `async resume()`: user gesture 後に audio context を再開する
- `setMasterVolume(v)`: 全体音量を設定する
- `setSeVolume(v)`: 単音 / SE bus の音量を設定する
- `setSeDelay(timeSec = 0.11, feedback = 0.26, wet = 0.22)`: 単音 / SE 用 delay を設定する
- `setSeReverb(send = 0.28, returnGain = 0.48)`: 単音 / SE 用 reverb を設定する
...
- `setSeReverbImpulse(config = {})`: 単音 / SE 用 reverb impulse を設定する
- `getSeReverbImpulseConfig()`: 単音 / SE 用 reverb impulse 設定を返す
- `setSeEnvelopePreset(name, config)`: 単音 / SE 用 envelope preset を登録する
- `getSeEnvelopePreset(name)`: 単音 / SE 用 envelope preset を返す
- `getSeEnvelopePresetList()`: 単音 / SE 用 envelope preset 一覧を返す
...
- `constructor()`: `ToneSynth` を継承した SE/BGM synth を作る
- `ensureContext()`: ToneSynth の context を確保し、BGM bus を追加する
- `buildBgmFxChain()`: BGM 用の delay / reverb chain を作る
- `async resume()`: user gesture 後に audio context を再開する
- `setMasterVolume(v)`: 全体音量を設定する
- `setBgmVolume(v)`: BGM 音量を設定する
- `setBgmDelay(timeSec = 0.18, feedback = 0.22, wet = 0.18)`: BGM 用 delay を設定する
- `setBgmReverb(send = 0.28, returnGain = 0.48)`: BGM 用 reverb を設定する
- `getImpulseKindList()`: 利用可能な impulse kind を返す
- `normalizeImpulseConfig(config = {})`: impulse 設定を正規化する
- `updateConvolverImpulse(convolver, config)`: convolver の impulse を更新する
...
- `playAudioBuffer(name, options = {})`: 登録済み音声素材を SE または BGM bus へ流して再生する
...
- `getStepParam(arr, step, _unusedFallback)`: 配列から step 対応値を返す
...
scene の上に card や status panel を重ねたいときは `OverlayPanel`、キー入力の代わりのボタンが欲しいときは `Touch`、と分けて考えると整理しやすくなります。
...
- `release(key)`: key を離す
- `beginFrame()`: 1 フレーム用の瞬間入力フラグを消す
- `captureFrameState(meta = {})`: 現在の入力状態を replay 用 snapshot として返す
- `applyFrameState(frame = {}, options = {})`: snapshot を現在の入力状態へ戻す
- `registerActionMap(map = {})`: action 名と key の対応を登録する
- `getActionMap()`: 現在の action map を object として返す
- `getAction(name)`: action が現在 active かを返す
- `wasActionPressed(name)`: その frame で action が押されたかを返す
- `wasActionReleased(name)`: その frame で action が離されたかを返す
- `pulseAction(key)`: 1 フレームだけ有効なアクションを作る
- `triggerAction(name)`: `pulseAction()` と同じ処理を行う別名
- `setTouchLayoutOptions(options = {})`: touch controls の配置既定値を設定する
- `shouldPreventDefaultForKeyboardEvent(event, key, preventDefault = true)`: keyboard の既定動作抑止可否を返す
- `setPointerPreventDefaultElement(element)`: pointer 既定動作抑止の対象 element を設定する
- `shouldPreventDefaultForPointerEvent(event, preventDefault = true, element = this.pointerPreventDefaultElement)`: pointer の既定動作抑止可否を返す
...
keyboard と touch の両方を使うサンプルでは、比較名を `16_タッチ機能と入力.md` の一覧に合わせて `"space"`、`"escape"`、`"f9"` のようにそろえると実装と文書の両方が追いやすくなります。
...
- `print()`: 空行を出力する
- `readUrlSync(filename)`: 同期的な URL 読み込みを行う
...
### `ShapeResource` / `Shape`
`ShapeResource` は共有可能な geometry と GPU buffer を持つ基底資源です。`Shape` はその共有資源を参照しながら、hidden 状態、material 差分、skeleton runtime などのインスタンス側の状態を持ちます。

- `ShapeResource.constructor(gpu)`: 頂点配列、index buffer、wireframe、bounding box、skin 情報を保持する共有資源を初期化する
- `ShapeResource.destroy(options = {})`: shared GPUBuffer と CPU 側配列を明示破棄する
...
- `Shape.destroy(options = {})`: shape instance の寿命を終了し、必要なら shared resource の破棄へつなげる

`Shape` を再利用する予定がない場合は `destroy()` を呼び、runtime から生成した scene 実体をまとめて片付けたい場合は `instantiated.destroy()`、その runtime 自体を今後使わない場合は `runtime.destroy()` を使います。GC による遅延回収に依存せず、不要になった resource は明示的に終了する運用が推奨されます。
