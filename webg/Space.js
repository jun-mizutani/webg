...
export default class Space {
  ...
  // 物理状態を持つノードを作成して登録する
  addPhysicsNode(parent_node, name, options = {}) {
    // PhysicsNode も scene graph 上では Node と同様に親子関係を持つため、
    // Space 側の登録フローは addNode() と同じ順序でそろえる
    const body = new PhysicsNode(parent_node, name, options);
    this.nodes.push(body);
    if (parent_node !== null) {
      parent_node.addChild(body);
    }
    return body;
  }
