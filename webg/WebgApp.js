...
export default class WebgApp {
  ...
  getGPU() {
    if (!this.screen) {
      throw new Error("WebgApp.getGPU() requires app.init() before accessing GPU");
    }
    return this.screen.getGPU();
  }
  ...
  async createParticleEmitter(options = {}) {
    ...
    await emitter.init(this.getGPU());
    return emitter;
  }
  ...
  async init() {
    ...
    if (!this.shader) {
      this.shader = new this.shaderClass(this.screen.getGPU());
    }
