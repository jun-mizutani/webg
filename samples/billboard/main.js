...
const start = async () => {
  ...
  const floorShape = createFloorShape(app.getGPU());
  const coreShape = createCoreShape(app.getGPU());
  ...
  const smokeTex = await createTexture(app.getGPU(), (tex) => {
  ...
  const debrisTex = await createTexture(app.getGPU(), (tex) => {
    tex.buildProceduralBillboardTexture({
      width: 96,
      height: 96,
      style: "debris",
  ...
  const billboard = new Billboard(app.getGPU(), PARTICLE_MAX);
  await billboard.init();
  billboard.setTexture(smokeTex);
  billboard.setOpacity(1.0);
  const shadowBillboard = new Billboard(app.getGPU(), PARTICLE_MAX);
