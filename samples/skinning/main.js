...
const start = async () => {
  ...
  const boneShader = new SmoothShader(app.getGPU());
  ...
  const staticShape = createStaticCylinder(app.getGPU());
  ...
  const softSkinned = createSkinnedCylinder(app.getGPU(), 3.0, CYLINDER_RADIUS, 12, 24, false);
  const hardSkinned = createSkinnedCylinder(app.getGPU(), 3.0, CYLINDER_RADIUS, 12, 24, true);
  const skinnedSets = [softSkinned, hardSkinned];
  for (let i = 0; i < skinnedSets.length; i++) {
    skinnedSets[i].shape.setShader(app.shader);
    skinnedSets[i].shape.skeleton.setBoneShape(createBoneShape(app.getGPU(), boneShader, boneSize));
