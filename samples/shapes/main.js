...
const start = async () => {
  ...
  const { rgba, width, height, colorTex } = await loadTextureFlipY(app.getGPU(), "../../webg/num256.png");
  const imageNormalTex = await createImageNormalTexture(app.getGPU(), rgba, width, height);
  const proceduralNormals = await createProceduralNormalTextures(app.getGPU());

  app.setDiagnosticsStage("build-shapes");
  const shapes = createShapes(app.getGPU());
  const state = {
    paused: false,
    surfaceModeKey: "t",
  ...
  const labels = new Message(app.getGPU());
