...
//
// 右の status は host / canvas / help / dialogue / pre panel / touch の
...
const dialogueEntries = [
  ...
  },
  {
    ...
    lines: [
      "help panel、pre panel、touch controls も同じ host を基準に配置されます",
...
const fixedPanelLines = [
  ...
  "",
  "この panel は showOverlayPanel({ format: \"pre\" }) で表示しています",
...
const showFixedPanel = (app, state) => {
  const panel = app.showOverlayPanel({
    id: "embeddedInfo",
    title: "embedded fixed-format panel",
    text: fixedPanelLines,
    format: "pre",
    scrollY: true,
    anchor: "bottom-left",
    offsetX: 16,
  ...
  state.fixedPanelNode = panel?.panel ?? null;
...
const toggleFixedPanel = (app, state) => {
  if (app.hasOverlayPanel("embeddedInfo")) {
    app.removeOverlayPanel("embeddedInfo");
    state.fixedPanelVisible = false;
    state.fixedPanelNode = null;
    return false;
  }
...
const toggleHelpPanel = (app, helpPanel) => {
  const nextCollapsed = helpPanel?.getState?.().collapsed !== true;
  helpPanel?.setCollapsed?.(nextCollapsed);
  return nextCollapsed !== true;
};
...
const updateExternalStatus = (app, helpPanel, state) => {
  ...
  statusEl.textContent = [
    ...
    `helpVisible: ${helpPanel?.getState?.().collapsed === true ? "no" : "yes"}`,
...
const updateHud = (app, helpPanel, state) => {
  ...
  app.message.setLines("status", [
    `help=${helpPanel.getState?.().collapsed === true ? "hide" : "show"}`,
...
const start = async () => {
  ...
  const helpPanel = app.showOverlayPanel(buildHelpPanelOptions({
    id: "embeddedHelp",
    lines: [
      "mouse drag: orbit",
