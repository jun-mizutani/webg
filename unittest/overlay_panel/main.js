...
const runAutoChecks = () => {
  ...
  check("WebgApp has no help preset facade", typeof app.buildHelpPanelOptions === "undefined");
  check("buildHelpPanelOptions preset import exists", typeof buildHelpPanelOptions === "function");
  try {
...
// help panel は独立 preset helper を通し、
// 折りたたみと再表示だけをシンプルに確認できる形へする
const createHelpPanel = () => {
  const options = buildHelpPanelOptions({
    id: "overlay-help",
    title: "Help",
    lines: [
...
const handleActions = () => {
  ...
  if (app.wasActionPressed("toggleHelp")) {
    const helpPanel = app.getOverlayPanel("overlay-help");
    helpPanel?.setCollapsed?.(helpPanel?.getState?.().collapsed !== true);
...
const start = async () => {
  ...
  const cubeShape = new Shape(app.getGPU());
