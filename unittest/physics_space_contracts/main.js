// ---------------------------------------------
// unittest/physics_space_contracts/main.js  2026/05/07
...
const runPlaneAndFrictionChecks = () => {
  ...
  check("zero-restitution plane suppresses rotating box bounce", maxBeamAngularSpeed < 180.0 && maxBeamUpwardVelocity < 2.0, formatValue({
    maxBeamAngularSpeed,
...
const finish = (app) => {
  const summary = failCount === 0
    ? `PASS physics_space_contracts (${passCount} checks)`
    : `FAIL physics_space_contracts (${failCount} failed / ${passCount} passed)`;
  app.setStatus(`${summary}\n\n${lines.join("\n")}`);
};

bootUnitTestApp({
  clearColor: [0.04, 0.06, 0.08, 1.0],
  initialStatus: "running physics_space_contracts..."
}, (app) => {
